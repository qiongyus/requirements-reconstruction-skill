...
同时钉死三件事：**范围**（整项目还是子系统）、**parity 基线版本**（tag 或 commit，全部证据引用它）、**工作仓库**（名称与路径，按下述约定，产出头声明）。用户没说清就问——分流、基线或工作仓库定错，后面全部白做。若用户明确要求不打断，默认「对等重写 + 整项目」，并在产出头显式声明这个假设。

**工作仓库约定**（三个重建 skill 共用；本文与姊妹 skill 中「重写仓库／当前工作仓库」在产出不进上游仓库时均指它）：与源项目克隆**同级**，重建期统一命名 `<upstream>-spec`——**不按分流区分**，因为三个分流的重建产出结构相同，且分流可能中途升级，按当下事实命名恒真；**必须是 git 仓库**（受治理真相层要有版本史）；`agent-spec init --workspace` 的 `knowledge/` 与约定根 `docs/reconstruction/` 都在其仓库根。parity 基线的源码快照用**源克隆的** git worktree 落在同级的 `<upstream>-baseline`，**不放进工作仓库**（避免 git 套 git）；源克隆内不落任何产物。若日后启动重写（进入 `construction-driven-development` 地界），同一仓库升格为重写仓库、按目标实现更名——git 目录改名保历史，spec 层原地不动。
