...
- **source**：指向本次重建的行为面×证据源矩阵文件（通常是 `docs/reconstruction/requirements/behavior-surface-matrix.md`），不是指向上游仓库——上游出处走 `## Source Trace`。
...
- **`## Requirements` 条款**：`[REQ-<id 同上>] <EARS 句式条款>`，modality（MUST/SHOULD/MAY）按 `SKILL.md`「Modality 映射表」推导，不凭手感选。**一条款一断言**：一个块里可以有多条 `[REQ-<id 同上>]` 行（id 相同，门 3 实测按多条 clause 正确解析），出现第二个 MUST/SHOULD 就另起一行，不要用 and 把两个断言并进一句——并写会被门 2 报 `requirement-single-statement` 与 `requirement-compound-clause` 两条 Warning，也违反 `references/requirement-quality.md` 第一节的 Singular。对等重写里最容易踩的是「MUST 写 stdout **且** MUST NOT 写文件」这种成对断言，下方示例块演示的正是拆开写。
...
- **空骨架块的占位值写法（本机实测逐属性核验）**：`id` 必须是 ASCII 字母数字段以单个连字符分隔（门 1 硬校验，实测尖括号/中文会直接报错 `invalid knowledge id`），骨架用 `REQ-YOUR-BEHAVIOR-ASSERTION` 这类全大写占位词而非尖括号；`title`（带引号）、`tags`（不带引号，但**不得含空格**——不带引号的属性值按空白分词，空格会把占位文字截断）、`source`（带引号）三者门 1 均不做字符校验，可用中文尖括号占位提示视觉上标出待替换处，骨架分别用 `"<替换为简短标题>"`、`<行为面类别>,<parity-class三选一>`、`"<按实际项目替换：通常是docs/reconstruction/requirements/behavior-surface-matrix.md>"`。
- **本文件的验证状态**：下方「填好的完整示例块」与「空骨架块」都是各自完整、起止 marker 齐全的独立块，`references/cli-gates.md`「门禁序列」的三条命令已对**两块合并抽取后的结果**联合实测——用扫描本文件全部 marker 区间的机械抽取方式一次即可同时取出两块；`requirements import` 两块都成功导入、无报错；`lint-knowledge --gate` 在两份文档上共报 2 条 Warning、0 Error，退出码 0（**2 条全部来自骨架块**的占位文本：`requirement-bcp14-keyword`/`requirement-weak-then`，均为预期——占位内容本就不构成合法条款，但不影响 Warning 级别的 gate 判定；**填好的示例块 0 findings**——2026-08-01 dogfood 复测发现它原先的单条复合条款会触发 `requirement-single-statement`/`requirement-compound-clause` 各一条，已按上文「一条款一断言」拆成两条并实测归零）；`requirements graph --gate` 退出码 0、`parse_errors` 为空（骨架块因保留占位式 `## Open Questions` 触发一条 `blocked-open-questions` warning 级 diagnostic，同样不卡门）。
...
```md
<!-- agent-spec:requirement id=REQ-FETCH-JSON-MODE title="fetch --json 输出契约" tags=cli,parity-contractual source=docs/reconstruction/requirements/behavior-surface-matrix.md -->
## Problem
...
[REQ-FETCH-JSON-MODE] When `--json` is supplied, the fetch command MUST write only a JSON document to stdout.

[REQ-FETCH-JSON-MODE] When `--json` is supplied, the fetch command MUST route diagnostics to stderr.
...
```md
<!-- agent-spec:requirement id=REQ-YOUR-BEHAVIOR-ASSERTION title="<替换为简短标题>" tags=<行为面类别>,<parity-class三选一> source="<按实际项目替换：通常是docs/reconstruction/requirements/behavior-surface-matrix.md>" -->
## Problem
